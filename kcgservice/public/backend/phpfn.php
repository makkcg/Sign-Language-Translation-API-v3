...
function GetFileUploadUrl($fld, $val, $resize = FALSE) {
	...
	if (!EmptyString($val)) {
		...
		$fileUrl = GetApiUrl(Config("API_FILE_ACTION"), "session=" . $sessionId . "&" . Config("TOKEN_NAME") . "=" . $CurrentToken);
		if ($fld->DataType == DATATYPE_BLOB) {
			$fn = $fileUrl;
...
function GetFileViewTag(&$fld, $val, $tooltip=FALSE) {
	...
	if (!EmptyString($val)) {
		if ($fld->DataType == DATATYPE_BLOB) {
			...
			$wrkfiles = explode(Config("MULTIPLE_UPLOAD_SEPARATOR"), $fld->Upload->DbValue);
		} else {
			$wrknames = [$val];
...
class AdvancedSecurity
{
	...
	protected function checkPermissions()
	{
		if (is_array($this->UserLevelPriv)) {
			...
			// Check if any import / lookup permissions enabled
			$hasImport = FALSE;
			$hasLookup = FALSE;
			foreach ($this->UserLevelPriv as $row) {
				$priv = $row[2];
				if (is_numeric($priv)) {
					if (($priv & ALLOW_IMPORT) == ALLOW_IMPORT)
						$hasImport = TRUE;
					if (($priv & ALLOW_LOOKUP) == ALLOW_LOOKUP)
						$hasLookup = TRUE;
					if ($hasImport && $hasLookup)
						break;
				}
			}
			if ($hasImport && $hasLookup)
			...
			// Import / Lookup permissions not setup, use Admin / List
			foreach ($this->UserLevelPriv as &$row) {
				$priv = &$row[2];
				if (is_numeric($priv)) {
					if (!$hasImport && ($priv & ALLOW_ADMIN) == ALLOW_ADMIN)
						$priv = $priv | ALLOW_IMPORT;
					if (!$hasLookup && ($priv & ALLOW_LIST) == ALLOW_LIST)
						$priv = $priv | ALLOW_LOOKUP;
				}
			}
		}
	}
		...
		if (Config("USER_TABLE") && !$this->isSysAdmin()) {
			...
			if (strval($user) != "") {
				...
				return ExecuteScalar("SELECT " . QuotedName($fldname, Config("USER_TABLE_DBID")) . " FROM " . Config("USER_TABLE") . " WHERE " .
					str_replace("%u", AdjustSql($user, Config("USER_TABLE_DBID")), Config("USER_NAME_FILTER")), $conn);
			}
		}
		return $info;
	}
