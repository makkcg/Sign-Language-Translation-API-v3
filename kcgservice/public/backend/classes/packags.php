...
/**
 * Table class for packags
 */
class packags extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'packags';
		$this->TableName = 'packags';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`packags`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('packags', 'packags', 'x_id', 'id', '`id`', '`id`', 21, 20, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// package
		$this->package = new DbField('packags', 'packags', 'x_package', 'package', '`package`', '`package`', 200, 191, -1, FALSE, '`package`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->package->Sortable = TRUE; // Allow sort
		$this->fields['package'] = &$this->package;

		// number_of_words
		$this->number_of_words = new DbField('packags', 'packags', 'x_number_of_words', 'number_of_words', '`number_of_words`', '`number_of_words`', 3, 11, -1, FALSE, '`number_of_words`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->number_of_words->Nullable = FALSE; // NOT NULL field
		$this->number_of_words->Sortable = TRUE; // Allow sort
		$this->number_of_words->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['number_of_words'] = &$this->number_of_words;

		// period_in_months
		$this->period_in_months = new DbField('packags', 'packags', 'x_period_in_months', 'period_in_months', '`period_in_months`', '`period_in_months`', 3, 11, -1, FALSE, '`period_in_months`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->period_in_months->Nullable = FALSE; // NOT NULL field
		$this->period_in_months->Sortable = TRUE; // Allow sort
		$this->period_in_months->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['period_in_months'] = &$this->period_in_months;

		// connection_points
		$this->connection_points = new DbField('packags', 'packags', 'x_connection_points', 'connection_points', '`connection_points`', '`connection_points`', 3, 11, -1, FALSE, '`connection_points`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// price
		$this->price = new DbField('packags', 'packags', 'x_price', 'price', '`price`', '`price`', 3, 11, -1, FALSE, '`price`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// created_at
		$this->created_at = new DbField('packags', 'packags', 'x_created_at', 'created_at', '`created_at`', CastDateFieldForLike("`created_at`", 0, "DB"), 135, 19, 0, FALSE, '`created_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->created_at->Sortable = TRUE; // Allow sort
		$this->created_at->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
		$this->fields['created_at'] = &$this->created_at;

		// updated_at
		$this->updated_at = new DbField('packags', 'packags', 'x_updated_at', 'updated_at', '`updated_at`', CastDateFieldForLike("`updated_at`", 0, "DB"), 135, 19, 0, FALSE, '`updated_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`packags`";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->number_of_words->DbValue = $row['number_of_words'];
		$this->period_in_months->DbValue = $row['period_in_months'];
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "packagsedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "packagsadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("packagsview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("packagsview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "packagsadd.php?" . $this->getUrlParm($parm);
		else
			$url = "packagsadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("packagsedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("packagsadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("packagsdelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->number_of_words->setDbValue($rs->fields('number_of_words'));
		$this->period_in_months->setDbValue($rs->fields('period_in_months'));
	...
	public function renderEditRow()
	{
		...
		$this->number_of_words->EditValue = $this->number_of_words->CurrentValue;
		...
		$this->period_in_months->EditValue = $this->period_in_months->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->number_of_words);
					$doc->exportCaption($this->period_in_months);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->number_of_words);
						$doc->exportField($this->period_in_months);
